		...
		<div class="right_col">
				<div class="db3" style="margin-top:0px" >
					...
					<div class="db3_content">
						<s:if test="#request.posibleInscripcion=='YaInscritoEliminar'">
							<h3><s:text name="oferta.general.eliminarInscripcion"/></h3>
							<sj:a id="botonEliminarInscripcion" cssClass="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover" button="false"> Pulsa aqui</sj:a>
								<sj:div id="capaEliminarInscripcion" bindOn="botonEliminarInscripcion">
									<s:form id="eliminarInscripcion" method="post" action="eliminarInscripcion">
										<s:textfield label="Introduzca (ELIMINAR)" name="palabraClave" />
										<s:hidden name="id" value="%{oferta.key.id}"/>
										<s:hidden name="idEmpresa" value="%{oferta.empresa.key.id}"/>
										<s:submit value="Confirmar" cssClass="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover"/>
									</s:form>
								</sj:div>
						</s:if>
						<s:if test="#request.posibleInscripcion!='YaInscritoEliminar' && #request.posibleInscripcion!='YaInscritoNoEliminar'">
							<h3><s:text name="oferta.general.inscribirse"/></h3>
							<s:if test="#request.posibleInscripcion=='PlazoCerrado'">
								<strong><u> <s:text name="oferta.plazoFinalizado"/></u> </strong>
							</s:if>
							<s:else>
								<s:url var="enlaceInscripcion" action="inscribirCandidato" encode="false">
									<s:param name="id" value="%{oferta.key.id}"/>
									<s:param name="idEmpresa" value="%{oferta.empresa.key.id}"/>
								</s:url>
								<s:a href="%{enlaceInscripcion}" cssClass="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover">Pulsa aqui para inscribirte </s:a>
							</s:else>
						</s:if>
				...
				<div class="db2">
					...
					<div class="db2_content">
						<div class="db2_content_container">
							<h3><s:text name="oferta.consultarResponsables.titulo"/></h3>
								<s:text name="oferta.consultarResponsables.texto"/><br/><br/><br/>
									<s:url var="urlVerResponsables" action="buscarResponsablesEmpresa">
										<s:param name="idEmpresa">
											<s:property value="%{oferta.empresa.key.id}"/>
										</s:param>
									</s:url>
								<s:a href="%{#urlVerResponsables}" cssClass="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover">Consultar responsables</s:a>
						</div>
					</div>
					<div class="bd2_bot">
