...
//			inscripcion = cs.getInscripciones(candidato);
    ...
    public String subirFoto() {
        ...
        String resultado = "ERROR";
        if (fichero != null) {
            log.info("Uploading not null file: ");
            candidato = getServiciosGlobales().cargarCandidato();
    ...
    public void mostrarFoto() {
        ...
        try {
            ...
            sos.write(candidato.getFoto().getBytes(), 0, candidato.getFoto().getBytes().length);
            sos.close();
        } catch (IOException e) {
            log.error("Unable to mostrarFoto", e);
        }
    }
...
//		}
    ...
    public String modificarDatosPersonalesCandidato() {
        ...
        if (fichero != null)
            candidato.setFoto(getServiciosGlobales().uploadedFileToBlob(fichero));
